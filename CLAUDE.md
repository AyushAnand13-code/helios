...
**Next (v2):** P0 (heartbeat), P1 (governed data path; `WEEKLY_SQL` deleted), and P2 (memory + suppression) have landed — four live MCP servers (stats, semantic, warehouse, report). Remaining, in priority order: **P3** `experiment-mcp` (a powered, sized experiment per finding — fulfils principle #4), **P4** the full 7-agent plan-execute-critique loop (promote the single brief + the Critic into Orchestrator/Monitor/.../Critic with per-agent tool allow-lists), **P5** depth/breadth (more stats-mcp tools, 47-metric coverage, forecast-based anomaly detection). Keep the labeled-eval gate green (≥85% top-1, 0 hallucinations) through every change.
