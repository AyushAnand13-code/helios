...
python -m helios.mcp.stats       # LIVE — decompose_change / significance_test / critique_decomposition / detect_anomaly / forecast
...
- **Forecast-based anomaly detection (v2 P5, in progress):** `helios/stats/forecast.py` — `detect_anomaly` (rolling-forecast + residual-sigma z-scores) and `forecast_next` (point forecast + prediction interval), dependency-light (stdlib + scipy, no prophet/statsmodels so lean CI runs it). `helios/diagnosis.most_anomalous_move` selects the week to diagnose by anomaly score on the conversion-RATE series — robust to partial boundary weeks (normal rate + low volume is NOT flagged), unlike `biggest_move`. The agent **Monitor uses it** (`--orchestrated`); exposed as stats-mcp tools `detect_anomaly`/`forecast`. Tested in `tests/test_forecast.py`. (Remaining P5 depth: cohort_retention/rfm_segment tools, broader metric coverage, and the Agent-SDK promotion — deferred.)
- **The 7-agent loop (v2 P4, complete):** `helios/agents/` is a deterministic plan-execute-critique orchestration over the existing tools — **role-based, not autonomous LLM agents** (the Agent-SDK version is a future extension). `roles.py` defines the 7 agents (Orchestrator/Monitor/Decompose/Diagnose/Prescribe/Narrator/Critic) each with an **enforced per-agent tool allow-list** (Bible §18.9 — the Narrator structurally cannot call `run_query`); `toolbox.py` is the chokepoint that checks the allow-list and records an audit trace; `orchestrator.py` runs Monitor → Decompose → Diagnose → **Critic (gate)** → Prescribe → Narrator → Orchestrator. `run_diagnosis` accepts injected `decompose`/`significance` hooks so those steps genuinely route through the Toolbox. Run via `python -m helios.run --orchestrated` — same brief plus an agent-run plan + tool trace; a REFUTE verdict **HELDs** the finding. Tested in `tests/test_agents.py` (allow-list enforcement, gate, trace).
...
python -m helios.run --orchestrated # same, driven through the 7-agent plan-execute-critique loop
...
**Next (v2):** P0–P4 have landed — **five live MCP servers** (stats, semantic, warehouse, report, experiment) + the 7-agent plan-execute-critique loop (`--orchestrated`), whole pipeline verified on real GA4. Remaining: **P5** depth/breadth — more stats-mcp tools (forecast/cohort/rfm), forecast-based anomaly detection instead of biggest-move, broader metric coverage — and, if you want true autonomy, promote the deterministic agent orchestration to the **Claude Agent SDK** (real LLM agents over the same allow-listed tools). Keep the labeled-eval gate green (≥85% top-1, 0 hallucinations) through every change.
