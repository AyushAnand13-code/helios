...
**Next (v2):** P0 (autonomous heartbeat) and P1 (governed data path — semantic-mcp + warehouse-mcp; `WEEKLY_SQL` deleted) have landed. Remaining, in priority order: **P2** memory (`report-mcp` save/recall + suppression + seasonality calendar so the heartbeat isn't spammy), **P3** `experiment-mcp` (powered experiment per finding), **P4** the full 7-agent plan-execute-critique loop, **P5** depth/breadth (more stats-mcp tools, 47-metric coverage, forecast-based anomaly detection). Keep the labeled-eval gate green (≥85% top-1, 0 hallucinations) through every change.
